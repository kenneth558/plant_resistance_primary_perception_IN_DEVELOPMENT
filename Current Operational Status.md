...
**One minimal-wattage resistor** of similar resistance as the plant tissue (1 Mohm works for me, and yes, I AM amazed that such a common value seems to work out fine) with some way to connect into the circuit.  The place this gets connected into the circuit is the reference Wheatstone bridge as the load resistance.  Note that the Wheatstone bridge leg that balances out the reference leg is called the signal leg, and its load resistance is the plant tissue under test, which can also be called Device Under Test (DUT).
